...
address:
  - code: a
    address: DATA2002 Group 005E06, School of Mathematics and Statistics Carslaw Building (F07) University of Sydney NSW 2006
...
lead_author_surname: Group 005E06

# Place eg a DOI URL or CRAN Package URL here
doi_footer: "https://github.sydney.edu.au/yyeh7345/005E06"

# Abstract
abstract: |
  In this report, we aimed to predict the age of abalone from the [UCI Abalone Dataset](https://archive.ics.uci.edu/dataset/1/abalone) by using only physical measurements. We found that a model constructed using only the variables that can be measured from live abalone was almost as accurate as the full model, and therefore recommend that this model be used for future research endeavours, due to its relative ease of use and animal welfare implications. 

# Optional: Acknowledgements
acknowledgements: |
  We used R version 4.3.1 [@RCoreTeam] to perform the calculations, along with the **tidyverse** suite of packages, including **ggplot2** for graphing [@tidyverse]. The **gridExtra**, **cowplot**, **ggfortify**, **kableExtra** and **corrplot** packages were also used to help produce the figures [@gridExtra; @cowplot; @ggfortify; @kableExtra; @corrplot2021]. Performance assessment was done using **caret** [@caret]. The report was created using the 'Pinp is not PNAS' template [@pinp].
...
<!-- some code to make longtable work from https://tex.stackexchange.com/questions/161431/how-to-solve-longtable-is-not-in-1-column-mode-error -->
...
```{r original_scatter, tidy = TRUE, warning = FALSE, message=FALSE, fig.cap="Original data scatter plot, comparing independent variables with rings", echo = FALSE, fig.height=3}
par( mfrow= c(2,4) )
...
```{r scaled_scatter, tidy = TRUE, warning = FALSE, fig.cap="Scaled data scatter plot, comparing independent variables with rings", message=FALSE, echo = FALSE}
par( mfrow= c(2,4) )
...
```{r correlation_matrix,message=FALSE, warning=FALSE, echo = FALSE, fig.height=2, fig.width = 2.5, fig.cap="Correlation heatmap (scaled)"}
...
#sjPlot::tab_model(step.back.aic.scaled, step.fwd.aic.scaled, show.ci = FALSE, show.aic = TRUE, dv.labels = c("Backward Model", "Forward Model"))$knitr %>% knitr::raw_html() %>% as_image(height = 3)
```

```{r original_assumptions, echo = FALSE, fig.height=1.5, fig.cap="Original model residual and QQ plots"}
...
#sjPlot::tab_model(step.back.aic.scaled.live, step.fwd.aic.scaled.live, show.ci = FALSE, show.aic = TRUE, dv.labels = c("Backward Live Abalone Model", "Forward Live Abalone Model"))$knitr %>% knitr::raw_html() |> as_image(height = 2)
```

```{r live_assumptions, echo = FALSE, fig.height=1.5, fig.cap="Live model residual and QQ plots"}
